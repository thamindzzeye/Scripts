...
foreach ($filename in $allFiles) {
  ...
  If($trim -eq 'y') {
    $begin = Read-Host -Prompt 'What is the start timestamp?'
    $duration = Read-Host -Prompt 'What is the duration?'
  ...
  $edit_video = Read-Host -Prompt 'Edit Video Gamma/Saturation?'
  $video_satisfied = 'n'
  If($edit_video -eq 'y') {
    $gamma = 1.0
    $saturation = 1.0
    ...
    While($video_satisfied -ne 'y') {
      ...
      $gamma = Read-Host -Prompt 'Gamma (0.1 - 10.0) (1.0 = normal)?'
      $saturation = Read-Host -Prompt 'Saturation (0.0 - 3.0) (1.0 = normal)?'
      $contrast = Read-Host -Prompt 'Contrast (-2.0 - 2.0) (1.0 = normal)?'
      $shouldPreview = Read-Host -Prompt 'Preview Video? (y/n)'
      If($shouldPreview -eq 'y') {
        ...
        $video_satisfied = Read-Host -Prompt 'Keep Video Settings? (y/n)'
      }
    }
