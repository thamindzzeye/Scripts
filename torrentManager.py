...
pathComplete = os.path.expanduser('~/Downloads/Torrents/Complete')
pathReadyToMove = os.path.expanduser('~/Downloads/Torrents/ReadyToMove')

def scanForRars(path):
	items = files = os.listdir(path)
	for item in items:
		
		itemPath = os.path.join(path, item)
		if os.path.isfile(itemPath): #file
			if "sample" in item.lower():
				print('was a sample')
				continue
			
			if isVideoFile(itemPath):
				print('moving! ' + itemPath)
# 				shutil.move(itemPath, os.path.join(pathReadyToMove, item))
			
		elif os.path.isdir(itemPath):
			print("folder: " + itemPath)
			scanForRars(itemPath)
...
def isVideoFile(path):
	...
	videoArray = ['.mkv', '.avi', '.mov', '.mpg', '.mp4', '.m4v']
	if extension in videoArray:
		return True
	else:
		return False
